...
タイトル・配信日・ジャケットは、それぞれ最も近い位置にある値を拾って曲と紐づけていますが、
誤って隣の曲のデータを拾わないよう、検索範囲は必ず「1つ前の曲のlinkcoreの位置」〜
「次の曲のlinkcoreの位置の直前」に区切っています(`nearest_bounded`)。実際に、この範囲制限が
無かった時期に新曲のジャケットが1つ前の曲の画像になってしまう不具合がありました。
