...
TuneCore側のサイト実装が変わると動かなくなる可能性があります。

### 曲名・配信日・ジャケットの取得方法

ページ内の `<script id="__NEXT_DATA__">` のJSONを読み取り(`scrape_from_next_data`)、
**リリース1件のオブジェクトの中にある値だけ**(`nameJa` / `releaseDate` / `linkcore` / `image`)を
その曲の情報として使います。文字位置の近さで推測する方式は、新曲の登録直後(曲名やジャケットが
まだTuneCore側に載っていない時期)に、隣の曲・アーティスト名・アーティスト写真を取り違える不具合を
繰り返したため、JSONが読めない場合の予備(`scrape_current_tracks_regex`)にのみ残しています。

- 曲名・配信日・ジャケットのどれかがまだ載っていない曲は、警告を出してスキップし、翌日以降の同期で取り込みます
- ジャケットは「その曲自身のリリースID(`/r<ID>/`)の画像」で、かつ正方形のものだけ採用します
  (`release_artwork_url` / `fetch_artwork`)。アーティスト写真などは採用されません
- 配信日が直近14日以内〜配信予定の曲は、毎回ジャケットを取り直し、保存済みの画像と違えば差し替えます
  (`refresh_recent_jackets`)。登録直後は暫定の画像で、後から本物に変わる場合に自動で追従するためです
